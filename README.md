...
The deployed site installs to a home screen on both Android and iPhone, with
its own icon and no address bar. There is no app store build and no APK: an
APK is Android-only, an iPhone needs a Mac and a paid Apple developer account,
and both would only wrap this same server. The manifest is generated by
`src/app/manifest.ts`, the icons by `src/app/app-icon/[size]`, which redraws
`/public/brand/logo-mark.svg` — replace that file and every icon follows.

- **Android, Chrome:** ⋮ → *Install app* (or *Add to Home screen*).
- **iPhone, Safari:** Share → *Add to Home Screen*. It must be Safari; Chrome
  on iOS cannot install a site.

No service worker ships, on purpose. Every page here is personalised and
server-rendered, so a cache that guessed wrong would show one member another
member's board. The cost is that the app needs a connection to open, which a
booking app does anyway.
